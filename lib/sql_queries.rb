...
"select projects.title, (sum(pledges.amount) - funding_goal) from projects join pledges on projects.id = pledges.project_id group by projects.title having sum(pledges.amount) >= projects.funding_goal;"
end
